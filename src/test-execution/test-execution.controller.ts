...
export class TestExecutionController {
  ...
  @Post()
  create(
    @Body() data: Prisma.TestExecutionCreateInput,
  ...
  async findOne(@Param('id') id: string): Promise<TestExecution | null> {
    const testExecution =
      await this.testExecutionService.getTestExecutionById(id);
    if (!testExecution) {
  ...
  async update(
    @Param('id') id: string,
    @Body() data: Prisma.TestExecutionUpdateInput,
  ): Promise<TestExecution> {
    try {
      const testExecution = await this.testExecutionService.updateTestExecution(
        id,
        data,
      );
      if (!testExecution) {
  ...
  async remove(@Param('id') id: string): Promise<TestExecution> {
    const testExecution =
      await this.testExecutionService.deleteTestExecution(id);
    if (!testExecution) {
      throw new NotFoundException();
    }
    return testExecution;
